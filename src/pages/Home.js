...
export default function Home() {
  useEffect(() => {
    // In SPA navigation, window "load" event does not fire again.
    // Force-hide template preloader when Home mounts.
    ...
    // Error handler to suppress external script errors
    const errorHandler = (event) => {
      if (event.filename && event.filename.includes('applock')) {
    ...
    const unhandledRejectionHandler = (event) => {
      if (event.reason && event.reason.toString().includes('offset')) {
        event.preventDefault();
      }
    };
    ...
    return () => {
      ...
      // Remove all applock scripts
      document.querySelectorAll('script[data-applock="true"]').forEach((el) => {
        try {
          el.remove();
        } catch (e) {
      ...
      // Clear jQuery and related globals to prevent script execution
      if (window.$) {
        try {
          window.$(window).off(".applockLoader .applockNav .applockBackToTop .counter");
          window.$(".navbar-nav a, .mouse-down").off(".applockAnchor");
          window.$(".back-to-top").off(".applockBackToTop");
          window.$(window).off("scroll.counter");
          window.$(document).off();
        } catch (e) {
          // Ignore
        }
          ...
          <div className="row align-items-center">
            <div className="col-lg-7 col-md-6">
              <div className="title-heading">
                <h1 className="heading mb-4" style={{ maxWidth: "760px", lineHeight: 1.28, fontSize: "56px", fontFamily: "Roboto, sans-serif", fontWeight: 900, letterSpacing: "0px", color: "#ffffff", textShadow: "0 2px 6px rgba(15, 23, 42, 0.55), 0 0 1px rgba(255,255,255,0.35), 0 0 0.6px rgba(255,255,255,0.35)" }}>Built for daily money control and real savings</h1>
                <div className="mb-4">
    ...
    <div className="container">
      ...
      <div className="row mt-5 align-items-center">
        <div className="col-md-7 mt-4 mt-sm-0 pt-2 pt-sm-0 order-2 order-md-1">
          <div className="about-app me-lg-4" style={{ maxWidth: "550px" }}>            <h4 className="title text-uppercase mt-2 mb-3">Introducing Ledger Talk</h4>
